...
export default function CommitmentForm({ options }: Props) {
  ...
  const basisDateParam = params[3] ?? '2022-10-01'
  const fiscalYearsParam = params[4] ? decodeURIComponent(params[4]) : '2022'
  const localCodesParam = params[5] ? decodeURIComponent(params[5]) : '11'
  ...
  function search(e: FormEvent<HTMLFormElement>) {
    ...
    const fiscalYears = selectedFiscalYears
      ...
      .sort()
      .join(',')
    const localCodes = selectedLocalGovs
      .map((option) => option.value)
      .sort()
      .join(',')
    const searchResultPage = `/commitment/edu/${basisDate}/${fiscalYears}/${localCodes}`
